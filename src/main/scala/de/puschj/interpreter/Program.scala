...
case class VariableProgram(private val stmts: List[Opt[Statement]]) extends Program {
  ...
  private def filterStatement(stmt: Statement, selectedFeatures: Set[String]): Statement = {
      stmt match {
            case ExpressionStmt(expr) => ExpressionStmt(filterExpression(expr, selectedFeatures))
        
            case Assert(expr) => Assert(filterExpression(expr, selectedFeatures))
            
            case Assignment(expr, value) => Assignment(filterExpression(expr, selectedFeatures), 
            ...
            case ClassDec(name, args, superClass, optConsts, optFields, optFuncDecs) =>
              val filteredArgs = args.filter(_.feature.evaluate(selectedFeatures))
                                     .map(a => Opt(True, a.entry))
              val filteredConsts = filterStatements(optConsts, selectedFeatures).asInstanceOf[List[Opt[Assignment]]]
              val filteredFields = filterStatements(optFields, selectedFeatures).asInstanceOf[List[Opt[Assignment]]]
  ...
  private def filterExpression(expr: Expression, selectedFeatures: Set[String]): Expression = {
    expr match {
  ...
  private def filterExpressions(exprs: List[Opt[Expression]], selectedFeatures: Set[String]): List[Opt[Expression]] = {
