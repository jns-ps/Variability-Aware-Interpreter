...
import de.puschj.interpreter.SourceCodePrettyPrinter
...
//    
//    val program: VariableProgram = parser.parseFile("program_latesplitting.txt")
//    val store = program.run()
...
//  }
  ...
  def testEarlyJoining() {
    ...
    val program: VariableProgram = parser.parseFile("program_earlyjoining.txt")
    val store = program.run()
